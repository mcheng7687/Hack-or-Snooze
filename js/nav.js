...
/* Show submission for new stories form */

function navSubmit(evt) {
  console.debug("navSubmit",evt);
  hidePageComponents();
  $addNewStoryForm.show();
}
...
$navFavor.on("click", navFavorites);
...
function updateUser () {
  ...
  $("#modify-user-name").val(currentUser.name);
}

$userProfile.on("click", updateUser);